...
const { loadEnvFiles, isProductionEnv, looksLikeRawFirebaseKey } = require('../utils/env');
...
function looksLikeFirebaseWebConfigJson(value = '') {
  const raw = String(value || '').trim();
  return raw.startsWith('{')
    && /"apiKey"\s*:/i.test(raw)
    && /"authDomain"\s*:/i.test(raw)
    && !/"type"\s*:\s*"service_account"/i.test(raw);
}
...
function decodeBase64Credential(value = '', label = 'FIREBASE_KEY_BASE64') {
  ...
  try {
    if (safeValue.startsWith('{')) {
      if (looksLikeFirebaseWebConfigJson(safeValue)) {
        throw new Error('Firebase Web Config Admin credential değildir; service-account JSON base64 gerekir');
      }
      return safeValue;
    }
    ...
    if (looksLikeFirebaseWebConfigJson(decoded)) {
      throw new Error('Firebase Web Config Admin credential değildir; service-account JSON base64 gerekir');
    }
...
function parseServiceAccountJson(raw = '', source = 'unknown') {
  ...
  if (looksLikeFirebaseWebConfigJson(value)) {
    throw new Error(`${source} Firebase Web Config içeriyor; Admin SDK için service-account JSON gerekir`);
  }
  try {
...
function resolveFirebaseServiceAccount() {
  ...
  if (direct) {
    if (looksLikeRawFirebaseKey(direct) || isProductionEnv(process.env)) {
      console.warn('[PlayMatrix][firebase] Legacy raw FIREBASE_KEY algılandı. FIREBASE_KEY_BASE64 veya FIREBASE_KEY_PATH standardına taşı.');
    }
...
function validateServiceAccount(serviceAccount = {}) {
  const missing = [];
  const privateKey = clean(serviceAccount.private_key);
  const privateKeyOk = /-----BEGIN (?:[A-Z0-9]+ )?PRIVATE KEY-----[\s\S]+-----END (?:[A-Z0-9]+ )?PRIVATE KEY-----/i.test(privateKey);
  ...
  if (!privateKeyOk) missing.push('private_key');
  if (missing.length) {
...
function markFirebaseUnavailable(error, source = '') {
  firebaseReady = false;
  firebaseCredentialSource = source || firebaseCredentialSource || 'development-memory-store';
  ...
  if (isProductionEnv(process.env)) {
    console.error(`[PlayMatrix][firebase] Production boot durduruldu. Firebase Admin zorunlu: ${message}`);
    throw firebaseInitError;
  }
  console.warn(`[PlayMatrix][firebase] Firebase Admin devre dışı; yalnızca development/test için REST auth + memory-store aktif: ${message}`);
}
...
(function initFirebase() {
  ...
  try {
    ...
    if (!resolved?.serviceAccount) {
      throw new Error('Firebase Admin credential bulunamadı. Render üzerinde FIREBASE_KEY_BASE64 veya FIREBASE_KEY_PATH tanımla.');
    }
