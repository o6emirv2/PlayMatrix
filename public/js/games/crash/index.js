...
const crashNoticeTimers = new Map();
    ...
    function getCrashNoticeDuration(type = 'info', requested = null) {
        ...
        const minimums = { error: 7600, warning: 6800, cashout: 6200, loss: 6200, success: 5200, xp: 5600, system: 5200, info: 4800 };
    ...
    function renderCashoutNotice(el, payload = {}) {
        ...
        img.alt = 'Avatar';
        img.src = payload.avatar || userInfo?.avatar || DEFAULT_AVATAR;
    ...
    function renderNoticeInto(el, payload = {}, timeout = 4800) {
        ...
        crashNoticeActive = { type: safeType, priority: getCrashNoticePriority(safeType), minUntil: Date.now() + Math.min(duration, safeType === 'error' ? 4200 : 2600) };
    ...
    function showCrashNotice(input = '', opts = {}) {
        ...
        if (Date.now() - lastShownAt < 1200) return;
...
function setBootActions({ showEnter = false, showRetry = false, enterLabel = 'CRASH OYNA', actionMode = 'continue' } = {}) {
    ...
    function crashOutcomeNoticeKey(input = {}, box = 1, outcome = 'cashout') {
        ...
        const round = String(input.roundId || currentRoundId || 'round');
        const betId = String(input.betId || input.id || `${round}:${normalizedBox}`);
        const xp = Number(input.xpAwarded ?? input.xpResult?.xpAwarded ?? 0) || 0;
        const mult = Number(input.cashoutMult ?? input.multiplier ?? 0) || 0;
        return `${betId}:${round}:${normalizedBox}:${outcome}:${mult.toFixed(2)}:${xp}`;
    }
    ...
    async function cashOut(box) {
        ...
        try {
            ...
            if (shouldNotifyCashout) showCrashNotice({ type: 'cashout', title: 'Çıkış alındı', avatar: userInfo.avatar || DEFAULT_AVATAR, message: userInfo.username || 'Sen', amount: `+${formatCompactMc(targetBet.win)}`, multiplier: `${Number(targetBet.cashoutMult || requestedAtMult).toFixed(2)}x`, xp: xpAwarded ? `+${xpAwarded} XP` : '', duration: 5200 });
    ...
    function maybeShowOutcomeNotice(player = {}) {
        ...
        if (player.cashed) {
            ...
            const xpText = xp?.xpAwarded > 0 ? ` • +${xp.xpAwarded} XP` : (xp?.reason === 'MANUAL_CASHOUT_BELOW_1_50_NO_XP' ? ' • XP için minimum 1.50x gerekir' : '');
            showCrashNotice({ type: 'cashout', title: 'Çıkış alındı', avatar: player.avatar || userInfo.avatar || DEFAULT_AVATAR, message: player.isMine ? (userInfo.username || 'Sen') : (player.username || 'Oyuncu'), amount: `+${formatCompactMc(win)}`, multiplier: `${mult.toFixed(2)}x`, xp: xp?.xpAwarded > 0 ? `+${xp.xpAwarded} XP` : '', duration: 5600 });
