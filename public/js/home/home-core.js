...
let pmAvatarSettingsRequested = false;
...
function userErrorText(error, fallback = 'İşlem tamamlanamadı.') {
  ...
  const map = {
    ...
    EMAIL_VERIFICATION_REQUIRED: 'Bu ödül için e-posta doğrulaması gerekiyor.',
...
function mountAvatar(host, { avatar = '', frame = 0, frameUrl = '', marketFrameId = '', frameKey = '', variant = '', size = 52, extraClass = '', topbar = false, useCurrentProfile = false } = {}) {
  ...
  if (!pmAvatarSettingsRequested && window.PMAvatar?.loadPublicSettings) {
    pmAvatarSettingsRequested = true;
    window.PMAvatar.loadPublicSettings().then(() => mountAvatar(host, { avatar, frame, frameUrl, marketFrameId, frameKey, variant, size, extraClass, topbar, useCurrentProfile })).catch(() => null);
  }
  try {
...
function renderHomeRecentWinners(items = []) {
  ...
  if (!winners.length) {
    host.innerHTML = '<div class="winners-empty-final"><i class="fa-solid fa-gem"></i><strong>Kazanan akışı hazırlanıyor.</strong><span>Oyun kazancı, promo kullanımı ve çark ödülleri geldiğinde bu alanda canlı olarak görünecek.</span></div>';
    return;
  }
...
async function loadHomeRecentWinners({ force = false } = {}) {
  ...
  let lastError = null;
  const candidates = [
    '/api/home/recent-activities?limit=15',
    '/api/home/recent-winners?limit=15',
    '/api/wheel/recent-winners?limit=15'
  ];
  for (const endpoint of candidates) {
    try {
      payload = await apiFetch(endpoint, { timeoutMs: 5000 }, false, false);
      const sample = Array.isArray(payload?.items) ? payload.items : Array.isArray(payload?.activities) ? payload.activities : Array.isArray(payload?.winners) ? payload.winners : [];
      if (sample.length || endpoint === candidates[candidates.length - 1]) break;
    } catch (error) {
      lastError = error;
      payload = null;
    }
  ...
  if (!payload) {
    payload = { items: [] };
    if (lastError) report('home.recentWinners.load', lastError, { endpoint: lastError?.endpoint || '/api/home/recent-activities', status: lastError?.status || 0 });
  }
...
async function loadLeaderboard(options = {}) {
  ...
  try {
    leaderboardPayload = await apiFetch('/api/leaderboard?limit=20', {}, false);
...
function renderMarketCategoryTabs(items = []) {
  ...
  host.replaceChildren(...MARKET_CATEGORIES.map(([key, label, icon]) => {
    ...
    button.addEventListener('click', () => {
      if (!hasItems) {
        showToast('Market', `Seçtiğiniz ${label.toLocaleLowerCase('tr-TR')} kategorisinde satışa açık ürün bulunmuyor.`, 'warning');
        return;
      }
...
async function requestEmailChangeLink() {
  ...
  try {
    ...
    setHelp('emailChangeHelp', '');
    showToast('E-posta', 'Güncelleme bağlantısı gönderildi.', 'success');
...
function bindEvents() {
  ...
  $('soundToggle')?.addEventListener('click', () => {
    const muted = document.body.classList.toggle('pm-muted');
...
function setupPickers() {
  avatarPicker = createAvatarPicker({
    ...
    onSelect: async ({ src }) => {
      ...
      currentProfile = { ...(currentProfile || blankProfile()), avatar: normalizeAvatarUrl(src, fallbackAvatar), marketAvatarId: '', selectedAvatar: '' };
      renderProfile();
      await apiFetch('/api/user/avatar', { method: 'POST', body: { avatar: currentProfile.avatar } }, true).catch((error) => {
        report('home.avatar.save', error);
        showToast('Avatar', userErrorText(error, 'Avatar kaydedilemedi.'), 'error');
      });
      showToast('Avatar', 'Avatar seçimi kaydedildi.', 'success');
    }
  });
