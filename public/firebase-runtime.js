...
function assertCurrentFirebaseContract(config = null, source = 'runtime') {
  if (!matchesCurrentFirebasePublicConfig(config)) {
    const error = new Error('Görünüm ayarları şu anda doğrulanamadı. Lütfen tekrar dene.');
...
export async function loadFirebaseWebConfig(options = {}) {
  ...
  try {
    ...
    if (options.required === false) return sanitizeFirebaseConfig(readStaticRuntimeConfig()?.firebase || null) || fallbackFirebaseConfig();
    const staticConfig = sanitizeFirebaseConfig(readStaticRuntimeConfig()?.firebase || null) || fallbackFirebaseConfig();
    if (staticConfig) return staticConfig;
    throw error;
  }
  const config = sanitizeFirebaseConfig(runtime?.firebase || null) || sanitizeFirebaseConfig(readStaticRuntimeConfig()?.firebase || null) || fallbackFirebaseConfig();
  if (config) {
