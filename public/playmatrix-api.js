(function () {
  ...
  async function fetchJson(path, options = {}) {
    ...
    if (!response.ok || payload?.ok === false) {
      ...
      const error = new Error(wasReplaced ? 'Devam etmek için giriş yapman gerekiyor.' : userFacingText(message, 'İşlem şu anda tamamlanamadı. Lütfen tekrar dene.'));
      if (wasReplaced) error.code = 'AUTH_REQUIRED';
