...
function getApiErrorCode(error = null) {
  const payload = error?.payload || error?.data || error?.body || {};
  return String(error?.rawCode || error?.code || payload?.code || payload?.error || error?.error || error?.message || error || '').toUpperCase();
}
...
function translatePistiErrorMessage(error = '') {
  ...
  const raw = String(error?.rawCode || payload.error || payload.code || error?.code || error?.message || error?.error || error || '').trim();
  const code = raw.toUpperCase();
  const status = Number(error?.status || error?.statusCode || payload.status || 0) || 0;
  if (code.includes('INSUFFICIENT_BALANCE') || code.includes('BALANCE_INSUFFICIENT')) return 'Bakiyen bu masa için yeterli değil.';
  if (code.includes('ROOM_NOT_FOUND')) return 'Masa bulunamadı. Yeni masa kurabilir veya lobiyi yenileyebilirsin.';
  if (code.includes('ROOM_CLOSED')) return 'Masa kapandı. Yeni masa kurabilirsin.';
  if (code.includes('ROOM_FULL')) return 'Masa dolu. Başka bir masa seçebilir veya yeni masa kurabilirsin.';
  if (code.includes('INVALID_BET')) return 'Bahis tutarı geçersiz. Lütfen 1.000 - 1.000.000 MC aralığında değer gir.';
  if (code.includes('NOT_YOUR_TURN')) return 'Sıra sende değil.';
  if (code.includes('STATE_VERSION_MISMATCH')) return 'Oyun senkronu yenilendi. Lütfen hamleni tekrar dene.';
...
function showPistiErrorModal(title = 'Reddedildi', error = '') {
  ...
  const friendly = translatePistiErrorMessage(error);
  const code = getApiErrorCode(error);
  const status = Number(error?.status || error?.statusCode || error?.payload?.status || 0) || 0;
  const isExpectedEmpty = code.includes('ROOM_NOT_FOUND') || status === 404;
  if (isExpectedEmpty && !currentRoomId) {
    lobbyErrorUntil = 0;
    if (list) list.innerHTML = '<div class="pm-pisti-empty">Masa bulunamadı. Yeni masa kurabilir veya tekrar deneyebilirsin.</div>';
    showLobbyNotice('Masa bulunamadı. Yeni masa kurabilir veya lobiyi yenileyebilirsin.', 'info');
    return;
  }
  lobbyErrorUntil = Date.now() + 6000;
  if (!currentRoomId && list) list.innerHTML = `<div class="pm-pisti-empty pm-pisti-empty--error">${escapeHTML(friendly)}</div>`;
  showMatrixModal(title, friendly, 'error');
}
...
async function fetchLobby(initial = false){
  ...
  if (initial || !fetchedRooms.length) {
    const list = document.getElementById('roomList');
    if (list) list.innerHTML = `<div class="pm-pisti-empty pm-pisti-empty--error">${escapeHTML(translatePistiErrorMessage(lastError))}</div>`;
  }
  if (initial) showLobbyNotice(translatePistiErrorMessage(lastError), 'warning', 'Tekrar Dene', () => fetchLobby(true).catch(() => null));
  return;
}
...
window.renderRoomListLocally = () => {
  ...
  if(filtered.length===0){
    ...
    list.innerHTML = recentError ? `<div class="pm-pisti-empty pm-pisti-empty--error">Lobi verisi alınamadı. Lütfen tekrar dene.</div>` : `<div class="pm-pisti-empty">Masa bulunamadı. Yeni masa kurabilir veya tekrar deneyebilirsin.</div>`;
    return;
  }
  let html = '';
  filtered.forEach(r => {
...
function syncUI(r){
  ...
  maybeShowPistiExtensionPrompt(r);
  if (r.status !== 'finished' && r.status !== 'abandoned') {
      clearPistiSocketNotice();
      if (r.lifecycle?.notice === 'extension-accepted') showGameNotice('Masa süresi iki oyuncu onayıyla 30 dakika uzatıldı.', 'info');
      else if (r.lifecycle?.notice === 'extension-pending' && r.extensionPrompt?.active) showGameNotice('60 dakika doldu. Devam kararı bekleniyor.', 'warning');
      else showGameNotice('');
  }
  if(r.status === 'finished' || r.status === 'abandoned'){ 
...
async function sendPistiExtensionDecision(accept) {
  const payload = { roomId: currentRoomId, accept: !!accept };
  try {
    await ensureGameplaySocket(false).catch(() => null);
    if (socket?.connected) {
      const ack = await new Promise((resolve, reject) => {
        let done = false;
        const timer = window.setTimeout(() => { if (!done) { done = true; reject(new Error('PISTI_EXTENSION_ACK_TIMEOUT')); } }, 4500);
        socket.emit('pisti:extend', payload, (response) => {
          if (done) return;
          done = true;
          window.clearTimeout(timer);
          if (response?.ok === false) reject(Object.assign(new Error(response.error || 'PISTI_EXTENSION_REJECTED'), response));
  ...
  return fetchAPI('/api/pisti-online/extend', 'POST', payload);
}

function maybeShowPistiExtensionPrompt(room = null) {
  const prompt = room?.extensionPrompt;
  if (!prompt?.active || prompt.myResponse !== null && prompt.myResponse !== undefined) return;
  const key = `${room.id}:${prompt.promptAt || 0}`;
  if (pistiExtensionPromptKey === key) return;
  pistiExtensionPromptKey = key;
  const titleEl = document.getElementById('matrixModalTitle');
  const descEl = document.getElementById('matrixModalDesc');
  const modal = document.getElementById('matrixModal');
  if (!titleEl || !descEl || !modal) {
    showGameNotice(prompt.message || '60 dakika doldu. Devam edilsin mi?', 'warning', 'DEVAM ET', async () => {
      const res = await sendPistiExtensionDecision(true).catch((error) => ({ ok: false, error }));
      if (res?.room) syncUI(res.room);
    });
    return;
  }
  titleEl.textContent = 'SÜRE UZATMA';
  descEl.replaceChildren();
  const text = document.createElement('span');
  text.className = 'pm-pisti-message-info';
  text.textContent = prompt.message || '60 dakika doldu. İki oyuncu kabul ederse masa 30 dakika uzatılacak.';
  const actions = document.createElement('div');
  actions.className = 'btn-dual';
  const reject = document.createElement('button');
  reject.type = 'button';
  reject.className = 'btn-no';
  reject.textContent = 'BİTİR';
  const accept = document.createElement('button');
  accept.type = 'button';
  accept.className = 'btn-yes';
  accept.textContent = 'DEVAM ET';
  reject.addEventListener('click', async () => {
    reject.disabled = true; accept.disabled = true;
    const res = await sendPistiExtensionDecision(false).catch((error) => ({ ok: false, error }));
    if (res?.room) syncUI(res.room);
    else resetToLobby();
    setModalActive('matrixModal', false);
  });
  accept.addEventListener('click', async () => {
    ...
    const res = await sendPistiExtensionDecision(true).catch((error) => ({ ok: false, error }));
    if (res?.room) syncUI(res.room);
    showGameNotice('Uzatma cevabın kaydedildi. Rakibin cevabı bekleniyor.', 'info');
    setModalActive('matrixModal', false);
  });
  actions.append(reject, accept);
  descEl.append(text, actions);
  modal.dataset.tone = 'warning';
  setModalActive('matrixModal', true);
}
...
window.playCard = async (idx, token) => {
  ...
  if (currentRoomState?.extensionPrompt?.active || currentRoomState?.lifecycle?.extensionState === 'pending') {
    maybeShowPistiExtensionPrompt(currentRoomState);
    showGameNotice('Masa süresi doldu. Devam etmek için uzatma kararını vermen gerekiyor.', 'warning');
    return;
  }
  ...
  try {
    ...
    if (playRes && playRes.ok && playRes.room) {
      ...
      clearTimeout(fallbackTimeout); 
      cardEl?.classList.remove('is-pending');
      if (getApiErrorCode(e).includes('ROOM_EXTENSION_PENDING')) {
        showGameNotice('Masa süresi doldu. Devam etmek için uzatma kararını vermen gerekiyor.', 'warning');
        maybeShowPistiExtensionPrompt(e?.payload?.room || currentRoomState);
      }
      syncUI(currentRoomState);
  }
};
