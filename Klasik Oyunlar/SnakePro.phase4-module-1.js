...
window.__PLAYMATRIX_API_URL__ = API_URL;
    ...
    const CLASSIC_GAME_TYPE = 'snakepro';
    const CLASSIC_LOGIN_URL = '/';
    const startupOverlay = document.getElementById('startup');
    const gameOverOverlay = document.getElementById('gameover');
    const finalScoreEl = document.getElementById('finalScore');
    const defaultStartupMarkup = startupOverlay ? startupOverlay.innerHTML : '';
    let classicRunId = '';
    ...
    function createClassicRunId() {
      ...
      return `snake_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`;
    }
    ...
    async function finishClassicRun(scoreValue = 0) {
      ...
      try {
        ...
        classicRunSubmitted = false;
        if (finalScoreEl) {
          finalScoreEl.innerText = `SKORUN: ${scoreValue} • Kaydedilemedi`;
        }
        return null;
      }
    }
    ...
    function applyClassicAccessState(user) {
      ...
      if (user) {
        ...
        if (startupOverlay) {
          startupOverlay.innerHTML = defaultStartupMarkup;
          if (!started) startupOverlay.style.display = 'flex';
        }
        updateBal();
        return;
      }
      ...
      if (startupOverlay) {
        ...
        startupOverlay.innerHTML = `
          <div class="pm-snake-login-title">GİRİŞ GEREKİYOR</div>
          <div class="pm-snake-login-desc">Snake Pro oynamak ve skorunu seviyene yazdırmak için önce giriş yap.</div>
          <a href="${CLASSIC_LOGIN_URL}" class="pm-snake-login-link">GİRİŞ YAP</a>
        `;
      }
    }

    [startupOverlay, gameOverOverlay].filter(Boolean).forEach((el) => {
...
const PM_REALTIME_PAGE_KEY = "snakepro";
