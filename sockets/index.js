...
const { logCaughtError, createRequestId } = require('../utils/logger');
...
function getGamePath(gameType = '') {
  return normalizeGameType(gameType) === 'pisti' ? '/Online Oyunlar/Pisti' : '/Online Oyunlar/Satranc';
}
...
async function createMatchmakingRoom(gameType, uidA, uidB) {
  ...
  return db.runTransaction(async (tx) => {
    ...
    return { roomId: roomRef.id, gameType: 'chess', gamePath: '/Online Oyunlar/Satranc' };
  });
}
...
module.exports = function initSockets(io, auth) {
  ...
  io.use(async (socket, next) => {
    try {
      ...
      socket.data = socket.data || {};
      socket.data.requestId = createRequestId(socket.handshake.headers?.['x-request-id'] || socket.handshake.auth?.requestId || socket.handshake.query?.requestId || '');
  ...
  io.on('connection', async (socket) => {
    ...
    const requestId = socket.data?.requestId || createRequestId('');
    trackUserOnline(uid);
    socket.emit('pm:connected', { requestId, socketId: socket.id });
    socket.join('crash');
    socket.join('lobby');
    ...
    socket.on('chat:dm_send', async (payload) => {
      try {
        ...
        captureError(error, { scope: 'socket', event: 'chat:dm_send', uid, requestId }).catch(() => null);
    ...
    socket.on('game:invite_send', async (payload) => {
      try {
        ...
        captureError(error, { scope: 'socket', event: 'game:invite_send', uid, requestId }).catch(() => null);
    ...
    socket.on('game:invite_response', async (payload) => {
      try {
        ...
        captureError(error, { scope: 'socket', event: 'game:invite_response', uid, requestId }).catch(() => null);
    ...
    socket.on('game:matchmake_join', async (data) => {
      try {
        ...
        captureError(error, { scope: 'socket', event: 'game:matchmake_join', uid, requestId }).catch(() => null);
        await removeUserFromQueue(uid);
