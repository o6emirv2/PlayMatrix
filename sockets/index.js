...
function mapInviteErrorPayload(error, fallbackMessage = 'Davet işlenemedi.') {
  const code = cleanStr(error?.message || error?.code || '', 48).toUpperCase();
  const known = {
    INVALID_INVITE_RESPONSE: 'Geçersiz davet yanıtı.',
    INVITE_NOT_FOUND: 'Davet bulunamadı veya temizlenmiş olabilir.',
    INVITE_FORBIDDEN: 'Bu davete yanıt verme yetkin yok.',
    INVITE_CLOSED: 'Davet artık açık değil.',
    INVITE_EXPIRED: 'Davetin süresi doldu.',
    ROOM_FULL: 'Oda dolduğu için davet gönderilemedi.',
    ROOM_CLOSED: 'Oda artık aktif değil.',
    ROOM_NOT_JOINABLE: 'Oda şu an davete uygun değil.',
    ROOM_INVALID: 'Oda verisi geçersiz.',
    INVALID_GAME: 'Davet oyunu geçersiz.',
    USER_BUSY: 'Hedef oyuncu şu an başka aktif oyunda.',
    NOT_FRIENDS: 'Sadece arkadaşlarını davet edebilirsin.'
  };
  return { code: code || 'INVITE_ERROR', message: known[code] || fallbackMessage };
}
...
async function syncPendingInvitesForSocket(socket, io, uid) {
  ...
  pendingInvites.forEach((invite) => {
    ...
    socket.emit('game:invite_receive', {
      ...
      deliveryStatus: invite.deliveryStatus || 'synced',
      statusMessage: invite.statusMessage || ''
    });
  });
  ...
  hostUpdates.forEach((invite) => {
    ...
    const basePayload = {
      ...
      closeReason: cleanStr(invite.closeReason || '', 48),
      statusMessage: cleanStr(invite.statusMessage || '', 240),
      expiresAt: safeNum(invite.expiresAt, 0)
    };
...
module.exports = function initSockets(io, auth) {
  ...
  io.on('connection', async (socket) => {
    ...
    socket.on('game:invite_send', async (payload) => {
      try {
        if (hitRateLimit('invite', uid, SOCKET_INVITE_WINDOW_MS, SOCKET_INVITE_MAX_PER_WINDOW)) return socket.emit('game:invite_error', { code: 'INVITE_RATE_LIMIT', closeReason: 'cooldown', retryAfterMs: SOCKET_INVITE_WINDOW_MS, cooldownWindowMs: SOCKET_INVITE_WINDOW_MS, maxPerWindow: SOCKET_INVITE_MAX_PER_WINDOW, message: 'Davet limiti aşıldı.' });
        ...
        if (!targetUid || targetUid === uid) return socket.emit('game:invite_error', { code: 'INVALID_TARGET', message: 'Geçersiz hedef oyuncu.' });
        if (!roomId || !gameKey) return socket.emit('game:invite_error', { code: 'INVALID_PAYLOAD', message: 'Davet bilgisi eksik.' });

        const areFriends = await ensureAcceptedFriendship(uid, targetUid);
        if (!areFriends) return socket.emit('game:invite_error', { code: 'NOT_FRIENDS', message: 'Sadece arkadaşlarını davet edebilirsin.' });
        ...
        await assertNoOtherActiveGame(targetUid, { allowGameType: gameKey, allowRoomId: roomId }).catch(() => { throw new Error('USER_BUSY'); });
        ...
        if (inviteConflict.kind === 'block') return socket.emit('game:invite_error', { code: 'INVITE_CONFLICT', closeReason: 'conflict', statusMessage: inviteConflict.message || 'Bu davet zaten bekliyor.', message: inviteConflict.message || 'Bu davet zaten bekliyor.' });
        const roomState = await evaluateInviteRoomState(inviteRoom, { roomId, gameKey, targetUid }, targetUid);
        if (!roomState.ok) {
          return socket.emit('game:invite_error', {
            code: roomState.code,
            closeReason: roomState.code.toLowerCase(),
            message: roomState.message || 'Oda şu an davete uygun değil.',
            roomId,
            gameKey
          });
        }
        ...
        const deliveryStatus = targetOnline ? 'delivered_realtime' : 'queued_offline';
        await colGameInvites().doc(invite.inviteId).set({
          updatedAt: nowMs(),
          deliveryStatus,
          targetDeliveredAt: targetOnline ? nowMs() : safeNum(invite.targetDeliveredAt, 0),
          statusMessage: targetOnline ? 'Davet gerçek zamanlı iletildi.' : 'Hedef oyuncu çevrimdışı; davet saklandı ve yeniden bağlanınca gösterilecek.'
        }, { merge: true }).catch(() => null);
        socket.emit('game:invite_sent', { inviteId: invite.inviteId, roomId, targetUid, targetName: target.username || target.fullName || invite.guestName || 'Oyuncu', gameKey, offline: !targetOnline, reused: inviteConflict.kind === 'reuse', expiresAt: invite.expiresAt, deliveryStatus, message: targetOnline ? 'Davet iletildi ve karşı tarafa gösterildi.' : 'Hedef oyuncu çevrimdışı. Davet saklandı ve çevrimiçi olduğunda yeniden gösterilecek.' });
      } catch (error) {
        captureError(error, { scope: 'socket', event: 'game:invite_send', uid }).catch(() => null);
        const mapped = mapInviteErrorPayload(error, error.message || 'Davet gönderilemedi.');
        socket.emit('game:invite_error', mapped);
      }
    });
    ...
    socket.on('game:invite_response', async (payload) => {
      try {
        ...
        if (!invite) return socket.emit('game:invite_error', { code: 'INVITE_NOT_FOUND', message: 'Davet bulunamadı veya süresi doldu.' });
        if (invite.targetUid !== uid) return socket.emit('game:invite_error', { code: 'INVITE_FORBIDDEN', message: 'Bu davete yanıt verme yetkiniz yok.' });
        if (safeNum(invite.expiresAt, 0) <= nowMs()) return socket.emit('game:invite_error', { code: 'INVITE_EXPIRED', closeReason: 'ttl_expired', message: 'Davetin süresi doldu.' });
        ...
        const mapped = mapInviteErrorPayload(error, error.message || 'Davet yanıtı işlenemedi.');
        socket.emit('game:invite_error', mapped);
      }
    });
