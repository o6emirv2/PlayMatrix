...
let curRoomId = null, myRole, isFin = false, canPlay = false, timerInt = null;
...
onAuthStateChanged(auth, async user => {
    if(!user) { window.location.href = '/'; return; } // Zorunlu Auth Kontrolü
    
    // Server'dan username al (Güvenli)
...
// XSS Korumalı (createElement/textContent) Lobi Listesi
function listenLobby() {
    if(unsubLobby) unsubLobby();
    // Güvenlik: Şifreler artık client'a gelmiyor. Public collection dinleniyor.
    ...
    unsubLobby = onSnapshot(q, snap => {
        ...
        snap.forEach(d => {
            ...
            if(r.status === 'waiting') {
                ...
                players.innerHTML = `<span style="color:var(--p1)">${r.p1Name.replace(/</g,"")}</span> <span style="color:#555">vs</span> <span style="color:var(--p2)">BEKLENİYOR...</span>`;
                ...
                const players = document.createElement('div'); players.className = "room-players";
                players.innerHTML = `<span style="color:var(--p1)">${r.p1Name.replace(/</g,"")}</span> <span style="color:#555">vs</span> <span style="color:var(--p2)">${(r.p2Name||"GİZLİ").replace(/</g,"")}</span>`;
                info.append(title, players);
...
// Bütün Buton Event Listener'ları (Inline kaldırma)
...
function enterGame(id) {
    ...
    sfx.lobby.pause(); sfx.rain.currentTime = 0;
    ...
    // GAME STATE Listener (Sadece oyunculara açık)
    unsubGame = onSnapshot(doc(db, "conquest_state", id), snap => {
        ...
        if(d.status === 'terminated' && !isFin) {
            isFin = true; showModalAlert("BAĞLANTI KOPTU", "Savaş durduruldu!", leaveToLobby); return;
        }

        // Fetch pub info for names
        ...
        if(d.status === 'playing' && d.endTimeMs) {
            ...
            timerInt = setInterval(() => {
                ...
                if(left === 0 && !isFin) {
                    ...
                    fetchAPI('/api/conquest/settle', 'POST', { roomId: id }).catch(()=>{});
                }
            }, 500);
        }
...
function finish(id, s1, s2, winner) {
    isFin = true; if(timerInt) clearInterval(timerInt); canPlay = false; 
...
async function leaveToLobby() {
    if (curRoomId) {
        if (unsubGame) unsubGame(); if (timerInt) clearInterval(timerInt); if (lightningTimer) clearTimeout(lightningTimer);
        try { await fetchAPI('/api/conquest/leave', 'POST', { roomId: curRoomId }); } catch (e) {}
    }
    curRoomId = null; isFin = false; canPlay = false; myRole = null;
    currentScene = 'lobby'; lightningFlash = 0;
    
    sfx.rain.pause(); sfx.thunder.pause();
    if (audioUnlocked) { sfx.lobby.currentTime = 0; sfx.lobby.play().catch(()=>{}); }
