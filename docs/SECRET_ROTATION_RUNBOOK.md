...
- If a service-account payload is missing required Admin fields in production, the web service fails fast before serving traffic. Replace the credential and redeploy; authenticated and database-backed APIs require a valid Firebase Admin credential.
...
- Placeholder/geçersiz `FIREBASE_KEY_BASE64` production ortamında boot hatasıdır.
- Varsa `FIREBASE_KEY_PATH`, split env veya legacy `FIREBASE_KEY` denenir.
- Production ortamında geçerli Admin credential yoksa servis memory-store fallback ile canlı kalmaz; deploy düzeltilene kadar başlatılmaz.
...
- Firebase Admin credential production ortamında hazır değilse backend başlatılmaz; public endpointler memory-store ile yanıltıcı veri üretmez.
- Auth session bootstrap ve database-backed endpointler production için geçerli Admin SDK credential gerektirir.

## Render 2026-04-25 Firebase Admin Production Boot Fix
...
- Production ortamında Admin credential yoksa servis kapanır ve trafik almadan hata verir.
- Development/test ortamında geçici memory-store yalnızca yerel geliştirme içindir.
- Login/session, profil, günlük çark ve veri yazan endpointler production için gerçek `FIREBASE_KEY_BASE64` veya `FIREBASE_KEY_PATH` service-account credential gerektirir.
- Kalıcı veri standardı Firestore Admin SDK yoludur; memory-store production çözümü değildir.
