...
## Faz 3 Runtime Sağlamlaştırma Standardı

- Render env doğrulaması deploy başında yapılır; hata veren zorunlu alanlarda servis kontrollü şekilde durur.
- Loglara yalnız sanitize edilmiş env raporu yazılır; `FIREBASE_KEY_BASE64`, service-account, admin ikinci/üçüncü faktör, hash/salt ve token değerleri hiçbir runtime logunda açık basılmaz.
- Firebase Admin credential eksik/geçersizse servis port açılışını engellemez; `rest-auth-memory-store` degraded modu aktif olur ve etkilenen modüller tek kontrollü log ile listelenir.
- Degraded modda etkilenen ana alanlar: auth session bootstrap, profil kalıcılığı, reward ledger, market inventory, game settlement, Crash history, Pişti/Satranç odaları, chat retention ve admin audit.
- Kalıcı veri garantisi için degraded mod kalıcı çözüm değildir; geçerli `FIREBASE_KEY_BASE64` veya `FIREBASE_KEY_PATH` zorunludur.
- Public runtime config yalnız public anahtarları döndürür; Admin credential, token, secret, hash/salt veya raw env değeri client’a verilmez.
- Public health çıktısı Firebase hata metnini açığa çıkarmaz; detaylı sağlık yüzeyi production’da yalnız admin guard arkasında açılır.
