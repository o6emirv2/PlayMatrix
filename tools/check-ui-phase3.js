...
for (const file of htmlFiles) {
  const source = read(file);
  if (!source.includes(viewport)) fail(`${file}: PlayMatrix viewport standardı bozulmuş.`);
  if (/initial-scale=1(?:,|"|\s)/.test(source)) fail(`${file}: yasaklı initial-scale=1 viewport bulundu.`);
}
...
for (const file of gameHtml) {
  ...
  mustContain(file, 'shell-enhancements.css', 'ortak shell CSS');
  mustContain(file, 'shell-enhancements.js', 'ortak shell JS');
  mustContain(file, 'avatar-frame.css', 'ortak avatar/çerçeve CSS');
  mustContain(file, 'avatar-frame.js', 'ortak avatar/çerçeve JS');
}

for (const file of ['public/admin/index.html', 'public/admin/admin.html', 'public/admin/health.html']) {
  if (exists(file)) {
...
if (!server.includes("app.use('/public', express.static(dir, publicStaticOptions))")) {
...
console.log('[check:ui-phase3] OK - ortak shell, PlayMatrix viewport, modal, topbar, avatar/çerçeve ve /public alias kontrolleri başarılı.');
