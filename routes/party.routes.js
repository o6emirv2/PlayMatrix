...
const { buildPartyInviteSnapshot } = require('../utils/inviteCenter');
...
function decoratePartyInvite(item = {}) {
  const ts = nowMs();
  const expiresAt = safeNum(item.expiresAt, 0);
  return {
    ...item,
    deliveryStatus: cleanStr(item.deliveryStatus || 'pending', 24) || 'pending',
    statusMessage: cleanStr(item.statusMessage || 'Yanıt bekleniyor.', 180) || 'Yanıt bekleniyor.',
    closeReason: cleanStr(item.closeReason || '', 32),
    expiresInMs: Math.max(0, expiresAt - ts)
  };
}
...
router.get('/party/me', verifyAuth, async (req, res) => {
  try {
    ...
    const [party, incomingInvitesRaw, outgoingInvitesRaw] = await Promise.all([
      getPartyByUid(uid),
      getPendingInvites(uid, 'incoming'),
      getPendingInvites(uid, 'outgoing')
    ]);
    const incomingInvites = incomingInvitesRaw.map((item) => decoratePartyInvite(item));
    const outgoingInvites = outgoingInvitesRaw.map((item) => decoratePartyInvite(item));
    const decoratedParty = await decoratePartyPayload(party);
    return res.json({
      ok: true,
      party: decoratedParty,
      incomingInvites,
      outgoingInvites,
      diagnostics: buildPartyInviteSnapshot({ incomingInvites, outgoingInvites, party: decoratedParty })
    });
  } catch (error) {
...
router.post('/party/invite', verifyAuth, profileLimiter, async (req, res) => {
  try {
    ...
    const payload = {
      ...
      statusMessage: 'Parti daveti gönderildi. Yanıt bekleniyor.',
      closeReason: '',
      createdAt: nowMs(),
      expiresAt: nowMs() + PARTY_INVITE_TTL_MS
    };
    await colPartyInvites().doc(inviteId).set(payload, { merge: true });
    await expireCompetingPartyInvites(targetUid, inviteId).catch(() => 0);
...
router.post('/party/respond', verifyAuth, profileLimiter, async (req, res) => {
  try {
    ...
    const result = await db.runTransaction(async (tx) => {
      ...
      if (action === 'decline') {
        tx.set(inviteRef, { status: 'declined', deliveryStatus: 'declined', statusMessage: 'Parti daveti reddedildi.', closeReason: 'declined', respondedAt: nowMs() }, { merge: true });
      ...
      if (memberUids.includes(uid)) {
        tx.set(inviteRef, { status: 'accepted', deliveryStatus: 'accepted', statusMessage: 'Parti daveti kabul edildi.', closeReason: 'accepted', respondedAt: nowMs() }, { merge: true });
