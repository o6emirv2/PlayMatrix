...
const { recordRewardLedger, summarizeRewardLedgerForUid } = require('../utils/rewardLedger');
const { buildProgressionSnapshot, getAccountLevel, getAccountXp, getVipMembershipScore, normalizeUserRankState } = require('../utils/progression');
...
const { getRewardRuntimeCatalog, getFixedRewardAmount, getWheelRewardPool } = require('../utils/rewardCenter');
const { sendApiSuccess, sendApiError } = require('../utils/apiResponse');
const { buildLeaderboardPayload } = require('../utils/leaderboardCenter');

const colUsers = () => db.collection('users');
const leaderboardCache = new TtlCache(15000, 24);
const leaderboardPayloadCache = new TtlCache(45000, 24);
const statsCenterCache = new TtlCache(30000, 100);
const userStatsCache = new TtlCache(60000, 200);
const homeShowcaseCache = new TtlCache(45000, 12);
...
async function settleLeaderboardTask(key, task, fallbackValue) {
  try {
    return { key, ok: true, value: await task() };
  } catch (error) {
    return { key, ok: false, value: fallbackValue, error: error instanceof Error ? error.message : 'unknown_error' };
  }
}

async function buildLeaderboardResponsePayload(viewerUid = '') {
  const tasks = await Promise.all([
    settleLeaderboardTask('levelTop', () => getLeaderboardTop('accountLevelScore', 5), []),
    settleLeaderboardTask('seasonTop', () => getLeaderboardTop('seasonRp', 5), []),
    settleLeaderboardTask('activityTop', () => getLeaderboardTop('monthlyActiveScore', 5), []),
    settleLeaderboardTask('vipTop', () => getVipLeaderboardTop(5), []),
    settleLeaderboardTask('chessTop', () => getLeaderboardTop('chessElo', 5), []),
    settleLeaderboardTask('pistiTop', () => getLeaderboardTop('pistiElo', 5), []),
    settleLeaderboardTask('selfLevel', () => viewerUid ? getLeaderboardSelfEntry(viewerUid, 'accountLevelScore') : null, null),
    settleLeaderboardTask('selfSeason', () => viewerUid ? getLeaderboardSelfEntry(viewerUid, 'seasonRp') : null, null),
    settleLeaderboardTask('selfActivity', () => viewerUid ? getLeaderboardSelfEntry(viewerUid, 'monthlyActiveScore') : null, null),
    settleLeaderboardTask('selfVip', () => viewerUid ? getVipSelfEntry(viewerUid) : null, null),
    settleLeaderboardTask('selfChess', () => viewerUid ? getLeaderboardSelfEntry(viewerUid, 'chessElo') : null, null),
    settleLeaderboardTask('selfPisti', () => viewerUid ? getLeaderboardSelfEntry(viewerUid, 'pistiElo') : null, null)
  ]);

  const taskMap = Object.fromEntries(tasks.map((item) => [item.key, item]));
  const degradedTabs = [];
  if (!taskMap.levelTop.ok) degradedTabs.push('level');
  if (!taskMap.seasonTop.ok) degradedTabs.push('season');
  if (!taskMap.activityTop.ok) degradedTabs.push('activity');
  if (!taskMap.vipTop.ok) degradedTabs.push('vip');
  if (!taskMap.chessTop.ok) degradedTabs.push('chess');
  if (!taskMap.pistiTop.ok) degradedTabs.push('pisti');

  const payload = buildLeaderboardPayload({
    levelTop: taskMap.levelTop.value,
    seasonTop: taskMap.seasonTop.value,
    activityTop: taskMap.activityTop.value,
    vipTop: taskMap.vipTop.value,
    chessTop: taskMap.chessTop.value,
    pistiTop: taskMap.pistiTop.value,
    self: {
      level: taskMap.selfLevel.value,
      rank: taskMap.selfSeason.value,
      season: taskMap.selfSeason.value,
      activity: taskMap.selfActivity.value,
      vip: taskMap.selfVip.value,
      chess: taskMap.selfChess.value,
      pisti: taskMap.selfPisti.value
    }
  }, {
    source: 'primary',
    degradedTabs
  ...
  payload.leaderboardMeta.viewerUid = cleanStr(viewerUid || '', 160) || null;
  payload.leaderboardMeta.selfResolved = viewerUid ? Boolean(taskMap.selfLevel.ok || taskMap.selfSeason.ok || taskMap.selfActivity.ok || taskMap.selfVip.ok || taskMap.selfChess.ok || taskMap.selfPisti.ok) : false;
  return payload;
}
...
router.get('/home/showcase', async (req, res) => {
  const cacheKey = 'home-showcase:public';
  try {
    const viewer = await tryVerifyOptionalAuth(req).catch(() => null);
    ...
    const games = getPublicGameCatalog();
    const catalogSummary = buildGameCatalogSummary(games);
    const rewardCatalogSummary = buildRewardCatalogSummary({ includePrivate: false, items: rewardCatalogConfig.items });
    ...
    const payload = {
      homeShowcase: {
        ...
        catalogSummary,
        rewards: {
          summary: rewardCatalogSummary,
          headline: rewardCatalogSummary.total > 0
            ? `${rewardCatalogSummary.total} ödül tanımı aktif`
            : 'Ödül vitrini hazırlanıyor',
          highlightLine: rewardCatalogSummary.total > 0
            ? `${rewardCatalogSummary.total} tanım · ${Array.isArray(rewardCatalogSummary.sources) ? rewardCatalogSummary.sources.length : 0} kaynak`
            : 'Kayıt, doğrulama, çark ve davet ödülleri tek katalogdan yönetilir.'
        },
        season: {
          timezone: resetSchedule.timezone,
          nextSeasonResetAt: resetSchedule.nextSeasonResetAt,
          nextSeasonResetLabel: resetSchedule.nextSeasonResetLabel,
          nextActivityRewardResetAt: resetSchedule.nextActivityRewardResetAt,
          nextActivityRewardResetLabel: resetSchedule.nextActivityRewardResetLabel
        },
        sync: {
          state: 'ready',
          source: 'primary',
          stale: false,
          viewerAuthenticated: !!viewer,
          generatedAt: nowMs()
        }
      }
    };
    homeShowcaseCache.set(cacheKey, payload, 45000);
    return sendApiSuccess(req, res, payload, { meta: { source: 'primary', public: true } });
  } catch (error) {
    const fallback = homeShowcaseCache.get(cacheKey);
    if (fallback) {
      return sendApiSuccess(req, res, {
        ...fallback,
        homeShowcase: {
          ...(fallback.homeShowcase || {}),
          sync: {
            ...((fallback.homeShowcase && fallback.homeShowcase.sync) || {}),
            state: 'stale',
            source: 'cache',
            stale: true,
            generatedAt: nowMs()
      ...
      }, { meta: { source: 'cache', stale: true, public: true } });
    }
    return sendApiError(req, res, 500, 'Ana sayfa vitrini yüklenemedi.', { code: 'HOME_SHOWCASE_LOAD_FAILED', retryable: true });
  }
});
...
router.get('/', verifyAuth, async (req, res) => {
  try {
    ...
    const signupRewardAmount = rewardRuntime.meta.registration.signupAmount;
    const emailRewardAmount = rewardRuntime.meta.registration.emailAmount;
    let uData = await db.runTransaction(async (tx) => {
      ...
      if (!snap.exists && !u.signupRewardClaimed) {
        balanceDelta += signupRewardAmount;
        updates.signupRewardClaimed = true;
        u.balance = safeNum(u.balance, 0) + signupRewardAmount;
      ...
      if (req.user.email_verified && !u.emailRewardClaimed && !isDisposableEmail(req.user.email)) {
        balanceDelta += emailRewardAmount;
        updates.emailRewardClaimed = true;
        u.balance = safeNum(u.balance, 0) + emailRewardAmount;
    ...
    Promise.allSettled([
      ...
      uData.grantedSignupReward ? recordRewardLedger({ uid: req.user.uid, amount: signupRewardAmount, source: 'signup_reward', referenceId: 'profile_me' }) : Promise.resolve(null),
      uData.grantedSignupReward ? createNotification({ uid: req.user.uid, type: 'reward', title: 'Hoş geldin ödülü', body: `Kayıt bonusu olarak ${signupRewardAmount.toLocaleString('tr-TR')} MC hesabına eklendi.` }) : Promise.resolve(null),
      uData.grantedEmailReward ? recordRewardLedger({ uid: req.user.uid, amount: emailRewardAmount, source: 'email_verify_reward', referenceId: 'profile_me' }) : Promise.resolve(null),
      uData.grantedEmailReward ? createNotification({ uid: req.user.uid, type: 'reward', title: 'E-posta doğrulama ödülü', body: `E-posta onayı için ${emailRewardAmount.toLocaleString('tr-TR')} MC hesabına eklendi.` }) : Promise.resolve(null)
    ]).catch(() => null);

    const publicUsername = await resolvePublicUsername(req.user.uid, uData.u);
    const progression = buildProgressionSnapshot(uData.u);
    const chatRetentionPolicy = await getChatRetentionPolicyConfig();
    const statsCenter = buildStatsCenterSnapshot(uData.u, { progression });
    const resetSchedule = buildResetScheduleSnapshot(new Date(), { chatRetention: chatRetentionPolicy });
    const safeUser = {
      ...uData.u,
    ...
    res.json({ ok: true, balance: safeNum(uData.u.balance, 0), user: safeUser, statsCenter, resetSchedule, toast: uData.toast });
  } catch (e) { res.json({ ok: false, error: e.message }); }
});


router.get('/stats-center', verifyAuth, async (req, res) => {
  const cacheKey = `stats-center:${cleanStr(req.user?.uid || '', 160)}`;
  try {
    ...
    const chatRetentionPolicy = await getChatRetentionPolicyConfig();
    const payload = {
      statsCenter: buildStatsCenterSnapshot(user, { progression }),
      resetSchedule: buildResetScheduleSnapshot(new Date(), { chatRetention: chatRetentionPolicy }),
      statsHealth: {
        state: 'ready',
        source: 'primary',
        stale: false,
    ...
    statsCenterCache.set(cacheKey, payload, 30000);
    return sendApiSuccess(req, res, payload, { meta: { source: 'primary' } });
  } catch (error) {
    const fallback = statsCenterCache.get(cacheKey);
    if (fallback) {
      return sendApiSuccess(req, res, {
        ...fallback,
        statsHealth: {
          ...(fallback.statsHealth || {}),
          state: 'stale',
          source: 'cache',
          stale: true
        }
    ...
    return sendApiError(req, res, 500, 'İstatistik merkezi yüklenemedi.', { code: 'STATS_CENTER_LOAD_FAILED', retryable: true });
  }
});

router.get('/reward-center', verifyAuth, async (req, res) => {
  try {
    const [summary, rewardRuntime, chatRetentionPolicy] = await Promise.all([
    ...
    return sendApiError(req, res, 500, 'Ödül merkezi yüklenemedi.', { code: 'REWARD_CENTER_LOAD_FAILED', retryable: true });
  }
});
...
router.get('/me/active-sessions', verifyAuth, async (req, res) => {
  try {
    const sessions = await listActiveSessionsForUid(req.user.uid, { includeBlackjack: true });
...
router.get('/leaderboard', async (req, res) => {
  ...
  const cacheKey = `leaderboard:${cleanStr(viewer?.uid || 'public', 160) || 'public'}`;
  try {
    const payload = await buildLeaderboardResponsePayload(viewer?.uid || '');
    leaderboardPayloadCache.set(cacheKey, payload, 45000);
    const meta = payload.leaderboardMeta || {};
    return sendApiSuccess(req, res, payload, {
      meta: {
        source: meta.source || 'primary',
        leaderboardState: meta.state || 'ready',
        degradedTabs: Array.isArray(meta.degradedTabs) ? meta.degradedTabs : []
      }
    });
  } catch (error) {
    const fallback = leaderboardPayloadCache.get(cacheKey);
    if (fallback) {
      const stalePayload = buildLeaderboardPayload(fallback, {
        source: 'cache',
        stale: true,
        degradedTabs: Array.isArray(fallback?.leaderboardMeta?.degradedTabs) ? fallback.leaderboardMeta.degradedTabs : []
      });
      stalePayload.self = fallback.self || {};
      stalePayload.leaderboardMeta.viewerUid = fallback?.leaderboardMeta?.viewerUid || cleanStr(viewer?.uid || '', 160) || null;
      stalePayload.leaderboardMeta.selfResolved = fallback?.leaderboardMeta?.selfResolved === true;
      return sendApiSuccess(req, res, stalePayload, {
        meta: {
          source: 'cache',
          stale: true,
          leaderboardState: stalePayload.leaderboardMeta.state,
          degradedTabs: stalePayload.leaderboardMeta.degradedTabs
        }
      });
    }
    return sendApiError(req, res, 500, 'Liderlik tablosu yüklenemedi.', { code: 'LEADERBOARD_LOAD_FAILED', retryable: true });
  }
});
...
router.get('/user-stats/:uid', verifyAuth, async (req, res) => {
  ...
  if (!uid || uid === 'undefined' || uid === 'null') return sendApiError(req, res, 400, 'Geçersiz kullanıcı kimliği.', { code: 'INVALID_UID', retryable: false });

  const cacheKey = `user-stats:${uid}`;
  try {
    const userDoc = await colUsers().doc(uid).get();
    if (!userDoc.exists) return sendApiError(req, res, 404, 'Kullanıcı bulunamadı.', { code: 'USER_NOT_FOUND', retryable: false });

    const [chatRetentionPolicy, _rewardCatalogConfig] = await Promise.all([
      getChatRetentionPolicyConfig(),
      getRewardCatalogConfig({ includePrivate: false })
    ]);
    const data = userDoc.data() || {};
    const progression = buildProgressionSnapshot(data);
    const statsCenter = buildStatsCenterSnapshot(data, { progression });
    const resetSchedule = buildResetScheduleSnapshot(new Date(), { chatRetention: chatRetentionPolicy });
    ...
    userStatsCache.set(cacheKey, payload, 60000);
    return sendApiSuccess(req, res, payload, { meta: { source: 'primary' } });
  } catch (error) {
    const fallback = userStatsCache.get(cacheKey);
    if (fallback) return sendApiSuccess(req, res, { ...fallback, stale: true, fallbackSource: 'cache' }, { meta: { source: 'cache', stale: true } });
    return sendApiError(req, res, 500, 'Sunucu hatası.', { code: 'USER_STATS_LOAD_FAILED', retryable: true });
  }
});
...
router.post('/wheel/spin', verifyAuth, async (req, res) => {
  try {
    ...
    const out = await db.runTransaction(async (tx) => {
      ...
      const rewards = getWheelRewardPool(rewardRuntime.map);
      if (!rewards.length) throw new Error('Çark ödül havuzu devre dışı.');
      ...
      return { index: rnd, prize: rewards[rnd], rewardPool: rewards };
    });
    Promise.allSettled([
...
router.post('/referral/claim', verifyAuth, async (req, res) => {
  try {
    ...
    const rewardRuntime = await getRewardRuntimeCatalog({ includePrivate: false });
    const REF_INVITER_REWARD = getFixedRewardAmount('referral_inviter', rewardRuntime.map, 50000);
    const REF_INVITEE_REWARD = getFixedRewardAmount('referral_invitee', rewardRuntime.map, 10000);
