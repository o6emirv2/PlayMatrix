  ...
  try {
    ...
    return res.json({
      ...
      data: {
        ...
        selectedFrame: getCanonicalSelectedFrame(data, { accountLevel: progression.accountLevel, defaultFrame: 0 }),
...
router.post('/update', verifyAuth, profileLimiter, async (req, res) => {
  try {
    ...
    const selectedFrameValue = Number.isFinite(Number(selectedFrame)) && Number(selectedFrame) >= 0 ? Math.max(0, Math.min(100, Math.floor(Number(selectedFrame)))) : undefined;
    res.json({ ok: true, selectedFrame: selectedFrameValue });
  } catch (e) { res.json({ ok: false, error: e.message }); }
});
