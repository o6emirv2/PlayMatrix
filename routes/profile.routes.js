...
async function formatLeaderboardUser(doc, rank = null, extra = {}) {
  ...
  const frameState = buildFrameState(canonical.selectedFrame, canonical.accountLevel);
  ...
  const balance = safeNum(data.balance, 0);
...
function buildFallbackUserStats(uid = '', user = {}) {
  const safeUid = cleanStr(uid || '', 160);
  const canonical = buildCanonicalUserState(user || {}, { defaultFrame: 0 });
  const progression = canonical.progression && typeof canonical.progression === 'object'
    ? canonical.progression
    : buildProgressionSnapshot(canonical);
  const frameState = buildFrameState(canonical.selectedFrame, canonical.accountLevel);
  const now = nowMs();
  return {
    uid: safeUid,
    username: cleanStr(user.username || user.displayName || '', 32) || 'Oyuncu',
    avatar: sanitizeAvatarForStorage(user.avatar) || DEFAULT_AVATAR,
    level: canonical.accountLevel,
    accountLevel: canonical.accountLevel,
    accountXp: canonical.accountXp,
    accountLevelScore: canonical.accountLevelScore,
    accountLevelProgressPct: progression.accountLevelProgressPct || 0,
    selectedFrame: canonical.selectedFrame,
    frameState,
    balance: safeNum(user.balance, 0),
    createdAt: getFirestoreTimestampMs(user.createdAt, now),
    lastLogin: getFirestoreTimestampMs(user.lastLogin || user.lastSeen || user.lastActiveAt, now),
    lastSeen: getFirestoreTimestampMs(user.lastSeen || user.lastActiveAt, now),
    monthlyActiveScore: canonical.monthlyActiveScore,
    progression,
    totalRounds: safeNum(user.totalRounds, 0),
    totalSpentMc: safeNum(user.totalSpentMc, 0),
    totalWins: safeNum(user.totalWins, 0),
    totalLosses: safeNum(user.totalLosses, 0),
    totalDraws: safeNum(user.totalDraws, 0),
    winRate: 0,
    winRatePct: 0,
    chessWins: safeNum(user.chessWins, 0),
    chessLosses: safeNum(user.chessLosses, 0),
    pistiWins: safeNum(user.pistiWins || user.pisti_wins, 0),
    pistiLosses: safeNum(user.pistiLosses || user.pisti_losses, 0),
    crashRounds: safeNum(user.crashRounds || user.crash_rounds, 0),
    crashWins: safeNum(user.crashWins || user.crash_wins, 0),
    crashLosses: safeNum(user.crashLosses || user.crash_losses, 0),
    classicStats: {},
    gameStats: {
      total: { rounds: safeNum(user.totalRounds, 0), wins: safeNum(user.totalWins, 0), losses: safeNum(user.totalLosses, 0), draws: safeNum(user.totalDraws, 0), winRatePct: 0 },
      chess: { wins: safeNum(user.chessWins, 0), losses: safeNum(user.chessLosses, 0), matches: safeNum(user.chessWins, 0) + safeNum(user.chessLosses, 0) },
      pisti: { wins: safeNum(user.pistiWins || user.pisti_wins, 0), losses: safeNum(user.pistiLosses || user.pisti_losses, 0), matches: safeNum(user.pistiWins || user.pisti_wins, 0) + safeNum(user.pistiLosses || user.pisti_losses, 0) },
      crash: { rounds: safeNum(user.crashRounds || user.crash_rounds, 0), wins: safeNum(user.crashWins || user.crash_wins, 0), losses: safeNum(user.crashLosses || user.crash_losses, 0) },
      classic: { totalRuns: 0, totalScore: 0, bestScore: 0 },
      history: { totalMatches: 0, wins: 0, losses: 0, draws: 0, byGame: {} }
    },
    recentGames: [],
    statistics: {
      accountLevel: canonical.accountLevel,
      ...
      crashWins: safeNum(user.crashWins || user.crash_wins, 0),
      crashLosses: safeNum(user.crashLosses || user.crash_losses, 0),
      classicTotals: { totalRuns: 0, totalScore: 0, bestScore: 0 },
      unreadMessages: safeNum(user.unread_messages, 0)
    }
  };
}
...
router.get('/', verifyAuth, async (req, res) => {
  try {
    ...
    const safeUser = {
      ...
      frameState,
      statistics: {
        balance: safeNum(uData.u.balance, 0),
        accountLevel: canonical.accountLevel,
        accountXp: canonical.accountXp,
        accountLevelScore: canonical.accountLevelScore,
        monthlyActiveScore: canonical.monthlyActiveScore,
...
router.get('/leaderboard', tryVerifyOptionalAuth, async (req, res) => {
  try {
    ...
    logCaughtError('profile.leaderboard', error, { requestId: req.requestId || null, uid: req.user?.uid || null }, 'error');
    return res.status(500).json({ ok: false, error: 'Liderlik tablosu yüklenemedi.', requestId: req.requestId || null });
  }
});
...
router.get('/user-stats/:uid', verifyAuth, async (req, res) => {
  try {
    ...
    if (!userDoc.exists) {
      if (!isFirebaseReady() && uid === cleanStr(req.user?.uid || '', 128)) {
        return res.json({ ok: true, degraded: true, data: buildFallbackUserStats(uid, { email: req.user.email || '' }) });
      }
    ...
    const frameState = buildFrameState(selectedFrame, progression.accountLevel);
    ...
    logCaughtError('profile.user_stats', error, { requestId: req.requestId || null, targetUid: req.params?.uid || '', uid: req.user?.uid || null }, 'error');
    if (isFirebaseAdminUnavailableError(error) && cleanStr(req.params?.uid || '', 128) === cleanStr(req.user?.uid || '', 128)) {
      return res.json({ ok: true, degraded: true, data: buildFallbackUserStats(req.user.uid, { email: req.user.email || '' }) });
    }
    return res.status(500).json({ ok: false, error: 'İstatistikler şu anda yüklenemedi.', requestId: req.requestId || null });
  }
});
  ...
  try {
    ...
    await db.runTransaction(async (tx) => {
      ...
      if (Number.isFinite(numericSelectedFrame) && numericSelectedFrame >= 0) {
        const safeSelectedFrame = normalizeSelectedFrameLevel(numericSelectedFrame);
        const maxUnlockedFrame = buildCanonicalUserState(u, { defaultFrame: 0 }).accountLevel;
        if (!isSelectedFrameUnlocked(safeSelectedFrame, maxUnlockedFrame)) {
          throw new Error(`Bu çerçeveyi kaydetmek için en az Seviye ${safeSelectedFrame} olmalısın.`);
        }
