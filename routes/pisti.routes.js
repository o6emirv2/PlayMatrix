...
const { PISTI_DISCONNECT_GRACE_MS, PISTI_RESULT_RETENTION_MS, SINGLE_PISTI_RESULT_RETENTION_MS, applyOnlinePistiCloseWindow, applySinglePistiCloseWindow } = require('../utils/roomLifecycle');
...
function applyPistiCloseWindow(room = {}, delayMs = PISTI_RESULT_RETENTION_MS) {
  return applyOnlinePistiCloseWindow(room, delayMs);
}
...
function schedulePistiRoomRemoval(roomId = '', delayMs = PISTI_RESULT_RETENTION_MS) {
  ...
  return Math.max(PISTI_DISCONNECT_GRACE_MS, safeNum(delayMs, PISTI_RESULT_RETENTION_MS));
}
...
function prepareRoomForSocket(room, viewerUid) {
  ...
  return {
    ...
    cleanupAt: safeNum(room.cleanupAt, 0), resumeAvailableUntil: safeNum(room.resumeAvailableUntil, 0), cleanupPolicy: cleanStr(room.cleanupPolicy || '', 32), canResume: ['waiting', 'playing'].includes(cleanStr(room.status || 'waiting', 24)) || safeNum(room.resumeAvailableUntil, 0) > nowMs(),
        ...
        const result = await db.runTransaction(async (tx) => {
            ...
            if (s.playerHand.length === 0 && s.botHand.length === 0) {
                if (s.deck.length >= 8) {
                    ...
                    applySinglePistiCloseWindow(s, SINGLE_PISTI_RESULT_RETENTION_MS);
