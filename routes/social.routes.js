...
const { getCanonicalSelectedFrame, buildFrameState } = require('../utils/accountState');
...
function buildSocialMemberPayload(uid = '', userData = {}, options = {}) {
  ...
  const selectedFrame = pickUserSelectedFrame({ ...userData, accountLevel: progression.accountLevel });
  const frameState = buildFrameState(selectedFrame, progression.accountLevel);
  const balance = safeNum(userData.balance, 0);
  return {
    uid,
