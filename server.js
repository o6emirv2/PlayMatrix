...
const { getAvatarFrameSettings } = require('./server/core/avatarFrameSettingsService');
...
app.use((req, res, next) => {
  ...
  res.on('finish', () => {
    ...
    const row = {
      ...
      message: `${req.method} ${safePath(req)} ${status}`,
      error: `${req.method} ${safePath(req)} ${status}`,
    ...
    addAdminLog('api.issue', { ...row, level: row.severity, source: row.area, category: row.scope, code: status >= 500 ? 'SERVER_ROUTE_ERROR' : 'UNEXPECTED_API_STATUS' });
    const log = status >= 500 ? console.error : console.warn;
    log(status >= 500 ? '[api:error]' : '[api:warning]', JSON.stringify({ method: row.method, path: row.path, status: row.status, ms: row.ms, requestId: row.requestId, game: row.game }));
  });
  next();
});
...
app.get('/api/avatar-frame/settings', async (_req, res) => {
  const settings = await getAvatarFrameSettings().catch(() => null);
  res.json({ ok: true, settings: settings || { version: 'playmatrix-avatar-frame-v2', variants: {} } });
});
