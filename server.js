...
// -------------------- State Doc Builders --------------------
function buildPublicDoc({ name, type, bet, status, hasPassword, playersCount, maxP, createdAtMs, p1, p2, p3, p4, p1_name, p2_name, p3_name, p4_name }) {
  return {
    name: cleanStr(name) || "Arena", type: cleanStr(type) || "2-52", bet: safeNum(bet, 0),
    status: cleanStr(status) || "waiting", hasPassword: !!hasPassword, playersCount: safeNum(playersCount, 0),
    ...
    createdAt: admin.firestore.FieldValue.serverTimestamp(), createdAtMs: safeNum(createdAtMs, nowMs()), updatedAtMs: nowMs()
  };
}

function buildInitialState({ type, deck, initialTable, bet }) {
  const state = { type: cleanStr(type) || "2-52", status: "waiting", bet: safeNum(bet, 0), deck: deck || [], table: initialTable || [], turn: null, lastCollector: null, pistiSignal: 0, finishedAtMs: null, settled: false };
  for (let i = 1; i <= 4; i++) { state[`p${i}`] = null; state[`p${i}_name`] = null; state[`p${i}_score`] = 0; state[`p${i}_count`] = 0; state[`p${i}_hand`] = []; }
  return state;
}
...
function pickRole(state, maxP) {
  ...
  if (maxP === 4) { if (!state.p3) return "p3"; if (!state.p4) return "p4"; }
  return null;
}
...
app.post('/api/profile/update', verifyAuth, async (req, res) => {
  try {
    ...
    await db.runTransaction(async (tx) => {
      ...
      // Kilitli Alanlar: Doluysa ASLA değişmez
...
// ==========================================
// PIŞTİ OYUN ENDPOINTLERİ (EKSİKSİZ KISIM)
// ==========================================
