...
const MAINTENANCE_KEYS = Object.freeze([
  ...
  'pattern-master', 'space-pro', 'snake-pro', 'matrix-siege', 'market', 'wheel', 'promo'
]);
...
function normalizeGameSlug(value = '') {
  ...
  if (/matrix[- ]?siege|matrixsiege/.test(raw)) return 'matrix-siege';
...
function areGamesEnabled(games = {}) {
  const normalized = normalizeMaintenanceGames(games);
  return !(normalized.general || normalized.system);
}
