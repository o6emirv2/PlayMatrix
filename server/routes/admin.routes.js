...
const { getAvatarFrameSettings, setAvatarFrameSettings, AVATAR_FRAME_VARIANTS } = require('../core/avatarFrameSettingsService');
...
router.get('/admin/avatar-frame/settings', async (_req, res) => {
  const settings = await getAvatarFrameSettings();
  res.json({ ok: true, variants: AVATAR_FRAME_VARIANTS, settings });
});

router.post('/admin/avatar-frame/settings', strictLimiter, requireAdminReauth, async (req, res) => {
  const result = await setAvatarFrameSettings(req.body || {}, { actor: adminActor(req) });
  logAdmin(req, 'admin.avatar-frame.settings.update', { variants: Object.keys(result.settings?.variants || {}) });
  res.json(result);
});
