...
async function leaderboardProfiles() { const { db } = fb(); if (!db) return []; try { const snap = await db.collection('users').limit(500).get(); return snap.docs.map(d => ({ uid: d.id, ...d.data() })); } catch { return []; } }
...
router.get('/leaderboard', async (req, res) => { const limit = Math.min(20, Math.max(1, Number(req.query?.limit || 20) || 20)); const profiles = await leaderboardProfiles(); const byLevel = [...profiles].sort((a,b)=> xpBig(b.accountXp ?? b.xp) > xpBig(a.accountXp ?? a.xp) ? 1 : xpBig(b.accountXp ?? b.xp) < xpBig(a.accountXp ?? a.xp) ? -1 : 0); const byActivity = [...profiles].sort((a,b)=>Number(b.monthlyActiveScore||0)-Number(a.monthlyActiveScore||0)); res.json({ ok: true, generatedAt: now(), limit, empty: profiles.length < 1, tabs: { level: { label: 'En Yüksek Hesap Seviyesi', metricKey: 'accountXp', items: lbItems(byLevel.slice(0,limit), 'level') }, activity: { label: 'En Çok Aktif Oyuncular', metricKey: 'monthlyActiveScore', items: lbItems(byActivity.slice(0,limit), 'activity') } } }); });
...
function systemNotification(title, message, icon = 'fa-bullhorn', extra = {}) {
  const row = { id: `sn_${Date.now()}_${crypto.randomBytes(4).toString('hex')}`, type: 'system', title: sanitizeText(title, 100), message: sanitizeText(message, 240), icon, at: now(), ...extra };
  pushMemoryList('notify:system', row, 30 * 86400000, 80);
  return row;
}
...
router.post('/notifications/system', requireAuth, requireAdmin, (req, res) => {
  ...
  const message = sanitizeText(req.body?.message || req.body?.text || '', 600);
  if (!title || !message) return res.status(400).json({ ok: false, error: 'TITLE_AND_MESSAGE_REQUIRED' });
  const row = systemNotification(title, message, s(req.body?.icon || 'fa-bullhorn', 40));
  res.json({ ok: true, row });
});
