...
router.post('/admin/market/item', requireAuth, requireAdmin, requireAdminReauth, async (req, res) => {
  ...
  try { await writeAdminAudit(req, 'market.item.upsert', { itemId: result?.item?.id || req.body?.id || req.body?.itemId || '', ok: result?.ok !== false }); } catch (_) {}
...
router.post('/admin/market/items/bulk', requireAuth, requireAdmin, requireAdminReauth, async (req, res) => {
  ...
  for (const row of rows) {
    try {
      ...
      if (result?.ok === false) failed.push({ id: row?.id || '', error: result.error || 'SAVE_FAILED', message: result.message || '' });
      else saved.push(result.item || { id: row?.id || '' });
    } catch (error) {
      failed.push({ id: row?.id || '', error: error?.code || error?.message || 'SAVE_FAILED', message: error?.message || '' });
    }
  }
