...
function publicIssueCopy(item = {}) {
  ...
  if (status >= 500) return { title: 'İşlem tamamlanamadı.', reason: 'Servis işlemi güvenli şekilde sonuçlandıramadı.', solution: 'Kayıt detayını inceleyip işlemi tekrar dene.' };
  if (status >= 400) return { title: 'İşlem başlatılamadı.', reason: 'Oturum, yetki veya işlem adımlarından biri tamamlanmadı.', solution: 'Gerekli adımları tamamlayıp tekrar dene.' };
...
function buildIssueCard(item = {}) {
  ...
  const detailText = [item.method, item.path || item.source || item.endpoint, item.status ? `Durum: ${item.status}` : '', item.requestId ? `İstek: ${item.requestId}` : '', item.scope || item.category || ''].filter(Boolean).join(' • ');
...
function marketAdminPreview(item = {}) {
  ...
  const asset = String(item.asset || item.preview || item.image || item.frameUrl || '');
...
async function saveMarketAdminItem(itemId = '') {
  ...
  const name = pick(`[data-market-name="${esc}"]`)?.value || id;
  const description = pick(`[data-market-description="${esc}"]`)?.value || '';
  const category = pick(`[data-market-category="${esc}"]`)?.value || 'frames';
  const price = Math.max(1, Math.trunc(Number(pick(`[data-market-price="${esc}"]`)?.value || 0) || 0));
  const stock = Math.max(0, Math.trunc(Number(pick(`[data-market-stock="${esc}"]`)?.value || 0) || 0));
  const active = !!pick(`[data-market-active="${esc}"]`)?.checked;
  const visible = !!pick(`[data-market-visible="${esc}"]`)?.checked;
  try {
    setStatus('marketAdminStatus', 'Market ürünü kaydediliyor...', 'info');
    await adminFetch('/api/admin/market/item', { method: 'POST', body: JSON.stringify({ id, name, title: name, description, category, price, stock, active, visible }) });
...
async function bulkSaveMarketAdminItems() {
  ...
  try {
    ...
    for (let i = 0; i < rows.length; i += 30) {
      ...
      if (Array.isArray(payload.failed)) payload.failed.forEach((f) => failures.push(`${f.id || 'ürün'}: ${f.error || 'kaydedilemedi'}`));
...
const ADMIN_MODAL_DEFS = Object.freeze([
  ...
  ['avatarFrame', 'fa-user-gear', 'Akıllı Avatar / Çerçeve Merkezi']
]);
...
function buildAvatarFramePanel() {
  const panel = el('section', 'panel stack');
  panel.id = 'avatarFrameAdminPanel';
  panel.innerHTML = `
    <div>
      <h2>AKILLI AVATAR / ÇERÇEVE YÖNETİM MERKEZİ</h2>
      <p class="lead">Variant bazlı avatar/frame scale, offset ve padding ayarlarını canlı render motoruyla uyumlu şekilde yönet.</p>
    </div>
    <div class="field-grid">
      <div class="field"><label for="afVariantSelect">Variant</label><select id="afVariantSelect"></select></div>
      <div class="field"><label for="afAvatarScale">Avatar Scale</label><input id="afAvatarScale" type="number" step="0.01" min="0.5" max="1.3" value="1" /></div>
      <div class="field"><label for="afFrameScale">Frame Scale</label><input id="afFrameScale" type="number" step="0.01" min="0.7" max="1.6" value="1" /></div>
      <div class="field"><label for="afAvatarOffsetX">Avatar X</label><input id="afAvatarOffsetX" type="number" step="1" min="-40" max="40" value="0" /></div>
      <div class="field"><label for="afAvatarOffsetY">Avatar Y</label><input id="afAvatarOffsetY" type="number" step="1" min="-40" max="40" value="0" /></div>
      <div class="field"><label for="afFrameOffsetX">Frame X</label><input id="afFrameOffsetX" type="number" step="1" min="-40" max="40" value="0" /></div>
      <div class="field"><label for="afFrameOffsetY">Frame Y</label><input id="afFrameOffsetY" type="number" step="1" min="-40" max="40" value="0" /></div>
      <div class="field"><label for="afInnerPadding">İç Padding</label><input id="afInnerPadding" type="number" step="1" min="0" max="40" value="6" /></div>
    </div>
    <div class="crash-control-hint">AnaSayfa topbar framesiz kalır. Oyun topbar frame kapatılamaz; bu merkez yalnızca hizalama/ölçek ayarı yapar.</div>
    <div class="action-row">
      <button id="avatarFramePreviewBtn" class="ghost" type="button">ÖNİZLEME YAP</button>
      <button id="avatarFrameSaveBtn" class="danger" type="button">KAYDET</button>
    </div>
    <div id="avatarFramePreview" class="summary-strip"></div>`;
  return panel;
}

function readAvatarFrameForm() {
  const variant = document.getElementById('afVariantSelect')?.value || 'leaderboard';
  return {
    variant,
    avatarScale: Number(document.getElementById('afAvatarScale')?.value || 1),
    frameScale: Number(document.getElementById('afFrameScale')?.value || 1),
    avatarOffsetX: Number(document.getElementById('afAvatarOffsetX')?.value || 0),
    avatarOffsetY: Number(document.getElementById('afAvatarOffsetY')?.value || 0),
    frameOffsetX: Number(document.getElementById('afFrameOffsetX')?.value || 0),
    frameOffsetY: Number(document.getElementById('afFrameOffsetY')?.value || 0),
    innerPadding: Number(document.getElementById('afInnerPadding')?.value || 6)
  };
}

const PLAYMATRIX_AVATAR_FRAME_VARIANTS = Object.freeze(['homeTopbar','leaderboard','accountModal','accountProfileCard','marketCard','crashTopbar','crashLivePanel','crashWinNotice','chessTopbar','chessGameCard','pistiTopbar','pistiScoreCard','snakeTopbar','spaceTopbar','patternTopbar']);
function avatarFramePreviewShell(variant = '') {
  const sizeMap = {
    homeTopbar: 40,
    leaderboard: 58,
    accountModal: 72,
    accountProfileCard: 84,
    marketCard: 78,
    crashTopbar: 50,
    crashLivePanel: 40,
    crashWinNotice: 64,
  ...
  const labels = {
    homeTopbar: ['AnaSayfa üst barı', 'Framesiz avatar slotu'],
    leaderboard: ['Liderlik kartı', 'Sıralama satırı gerçek kart ölçüsü'],
    accountModal: ['Hesabım modalı', 'Modal profil görünümü'],
    accountProfileCard: ['Profil kartı', 'Büyük hesap kartı görünümü'],
    marketCard: ['Market kartı', 'Ürün önizleme slotu'],
    crashTopbar: ['CRASH', 'Oyun üst barı'],
    crashLivePanel: ['Canlı Tur', 'Cashout / loss satırı'],
    crashWinNotice: ['Crash kazanç bildirimi', 'Özel kazanç kartı'],
    chessTopbar: ['SATRANÇ', 'Oyun üst barı'],
    chessGameCard: ['Satranç oyun kartı', 'Oyuncu / rakip alanı'],
    pistiTopbar: ['PİŞTİ', 'Oyun üst barı'],
    pistiScoreCard: ['Pişti skor kartı', 'Masa skor satırı'],
    snakeTopbar: ['SNAKE PRO', 'Oyun üst barı'],
    spaceTopbar: ['SPACE PRO', 'Oyun üst barı'],
    patternTopbar: ['PATTERN MASTER', 'Oyun üst barı']
  };
  const [title, subtitle] = labels[variant] || [variant, 'Canlı component slotu'];
  const shell = el('article', `af-real-preview af-real-preview--${variant}`);
  shell.dataset.variant = variant;
  const top = el('div', 'af-preview-top');
  top.append(el('strong', '', title), el('span', '', subtitle));
  const surface = el('div', `af-preview-surface af-preview-surface--${variant}`);
  const component = el('div', `af-component-preview af-component-preview--${variant}`);
  const host = el('div', 'af-preview-avatar-slot pm-avatar-live-preview-host');
  host.dataset.previewSize = String(sizeMap[variant] || 56);
  if (variant === 'leaderboard') {
    component.innerHTML = '<div class="af-rank-badge">#1</div><div class="af-player-copy"><b>PlayMatrix Oyuncu</b><span>Seviye 54 • 4.000 XP</span></div>';
    component.prepend(host);
  } else if (variant === 'marketCard') {
    component.innerHTML = '<div class="af-player-copy"><b>Market Çerçevesi</b><span>Canlı market kartı slotu</span></div><span class="af-price-chip">MC</span>';
    component.prepend(host);
  } else if (variant === 'crashLivePanel') {
    component.innerHTML = '<div class="af-player-copy"><b>Oyuncu cashout</b><span>2.14x • 12.500 MC</span></div>';
    component.prepend(host);
  } else if (variant === 'crashWinNotice') {
    component.innerHTML = '<div class="af-player-copy"><b>Kazanç bildirimi</b><span>MC + XP + çarpan alanı</span></div>';
    component.prepend(host);
  } else if (variant === 'chessGameCard' || variant === 'pistiScoreCard') {
    component.innerHTML = '<div class="af-player-copy"><b>Oyuncu</b><span>Skor / sıra / masa bilgisi</span></div><div class="af-score-chip">AKTİF</div>';
    component.prepend(host);
  } else if (/Topbar$/.test(variant) || variant === 'homeTopbar') {
    component.innerHTML = `<div class="af-player-copy"><b>${title}</b><span>${variant === 'homeTopbar' ? 'Frame kapalı' : 'Frame zorunlu açık'}</span></div><div class="af-score-chip">TOPBAR</div>`;
    component.prepend(host);
  } else {
    component.innerHTML = '<div class="af-player-copy"><b>Profil görünümü</b><span>Canlı modal/kart slotu</span></div>';
    component.prepend(host);
  }
  surface.appendChild(component);
  shell.append(top, surface);
  return { shell, host, sizePx: Number(host.dataset.previewSize || 56) || 56 };
}

function renderAvatarFramePreview() {
  const preview = document.getElementById('avatarFramePreview');
  if (!preview) return;
  const form = readAvatarFrameForm();
  preview.innerHTML = '';
  const { shell, host, sizePx } = avatarFramePreviewShell(form.variant);
  ...
  const runtimeSetting = {
    ...
    innerPadding: form.innerPadding,
    outerPadding: 0,
    overflow: form.variant === 'homeTopbar' ? 'hidden' : 'visible',
  ...
  if (window.PMAvatar?.mount) {
    window.PMAvatar.mount(host, {
      ...
      level: form.variant === 'homeTopbar' ? 0 : 100,
      exactFrameIndex: form.variant === 'homeTopbar' ? 0 : 18,
      frameUrl: form.variant === 'homeTopbar' ? '' : '/public/assets/frames/frame-18.png',
      variant: form.variant,
      variantSetting: runtimeSetting,
      sizePx,
      extraClass: 'pm-avatar--admin-live-preview',
  ...
  avatar.className = 'af-preview-avatar';
  avatar.alt = 'Avatar önizleme';
  avatar.src = BRAND_LOGO_URL;
  host.appendChild(avatar);
}

async function loadAvatarFrameAdmin() {
  const select = document.getElementById('afVariantSelect');
  if (!select || select.dataset.loaded === '1') return;
  const payload = await adminFetch('/api/admin/avatar-frame/settings').catch(() => ({ variants: PLAYMATRIX_AVATAR_FRAME_VARIANTS, settings: {} }));
  const variants = Array.from(new Set([...(payload.variants || []), ...PLAYMATRIX_AVATAR_FRAME_VARIANTS])).filter((variant) => PLAYMATRIX_AVATAR_FRAME_VARIANTS.includes(variant));
  select.replaceChildren(...variants.map((variant) => { const opt = document.createElement('option'); opt.value = variant; opt.textContent = variant === 'homeTopbar' ? `${variant} (framesiz avatar hizası)` : variant; return opt; }));
  select.dataset.loaded = '1';
  setStatus('avatarFrameStatus', 'Avatar / çerçeve ayarları yüklendi.', 'ok');
  renderAvatarFramePreview();
}

async function saveAvatarFrameAdmin() {
  const form = readAvatarFrameForm();
  const payload = { variants: { [form.variant]: form } };
  await adminFetch('/api/admin/avatar-frame/settings', { method: 'POST', body: JSON.stringify(payload) });
  setStatus('avatarFrameStatus', 'Avatar / çerçeve ayarı kaydedildi.', 'ok');
}
...
function installAdminModalSystem() {
  ...
  const avatarFramePanel = buildAvatarFramePanel();
  app.appendChild(userInfoPanel);
  app.appendChild(avatarFramePanel);
  const entries = { reset: resetPanel, maintenance: maintenancePanel, crash: crashPanel, wheel: wheelPanel, market: marketPanel, restrict: restrictPanel, reward: rewardPanel, rewardAll: rewardAllPanel, promo: promoPanel, userInfo: userInfoPanel, issues: issuesPanel, avatarFrame: avatarFramePanel };
...
function openAdminPanelModal(key = '') {
  ...
  if (String(key || '') === 'avatarFrame') loadAvatarFrameAdmin().catch((error) => setStatus('avatarFrameStatus', error.message || 'Ayarlar yüklenemedi.', 'error'));
...
function handleDashboardClick(event) {
  ...
  if (button.id === 'avatarFramePreviewBtn') return renderAvatarFramePreview();
  if (button.id === 'avatarFrameSaveBtn') return saveAvatarFrameAdmin().catch((error) => setStatus('avatarFrameStatus', error.message || 'Avatar / çerçeve ayarı kaydedilemedi.', 'error'));
