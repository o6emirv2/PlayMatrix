...
async function verifyAuth(req, res, next) {
  ...
  try {
    if (sessionToken) {
      ...
      clearSessionCookieIfPossible(res);
      if (!bearerToken) {
        return res.status(401).json({ ok: false, redirect: true, code: session?.reason || 'INVALID_SESSION', error: 'Geçersiz oturum.', requestId: req.requestId || null });
      }
    }

    if (!bearerToken) {
      return res.status(401).json({ ok: false, redirect: true, error: 'Oturum yok.', requestId: req.requestId || null });
    }
    ...
    clearSessionCookieIfPossible(res);
    logCaughtError('auth.verify', error, { requestId: req.requestId || null, route: req.originalUrl || req.url || '' });
    return res.status(401).json({ ok: false, redirect: true, error: 'Geçersiz token.', requestId: req.requestId || null });
  }
}
