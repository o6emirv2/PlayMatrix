...
function buildLimiter(windowMs, max, message, overrides = {}) {
  return rateLimit({
    ...
    handler: (req, res) => {
      const payload = typeof message === 'object' && message ? { ...message } : { ok: false, error: String(message || 'Limit aşıldı.') };
      return res.status(429).json({
        ok: payload.ok === undefined ? false : payload.ok,
        error: payload.error || 'Limit aşıldı.',
        requestId: req.requestId || null,
        retryAfterSec: Math.ceil(Number(windowMs || 0) / 1000)
      });
    },
    ...overrides
  });
