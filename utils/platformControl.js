...
const { buildRegistrationRewardSnapshot } = require('./rewardCenter');
...
function buildPlatformControlSnapshot(options = {}) {
  ...
  const rewardCatalogSummary = options.rewardCatalogSummary && typeof options.rewardCatalogSummary === 'object'
    ...
    : {};
  const chatRetention = options.chatRetention && typeof options.chatRetention === 'object'
  ...
  const roomHealth = options.roomHealth && typeof options.roomHealth === 'object' ? options.roomHealth : null;
  const registrationRewards = buildRegistrationRewardSnapshot(options.rewardCatalogItems || []);
  ...
  return {
    ...
    rewards: {
      registrationFlowLabel: registrationRewards.label,
