...
function getRuntimeEnvReport(env = process.env) {
  ...
  const credentialSignals = {
    FIREBASE_KEY_BASE64: hasValue(env, 'FIREBASE_KEY_BASE64'),
    FIREBASE_KEY_PATH: hasValue(env, 'FIREBASE_KEY_PATH'),
    FIREBASE_SPLIT_ENV: hasAnyValue(env, ['FIREBASE_PRIVATE_KEY', 'FIREBASE_PRIVATE_KEY_BASE64', 'GOOGLE_PRIVATE_KEY'])
      && hasAnyValue(env, ['FIREBASE_CLIENT_EMAIL', 'FIREBASE_SERVICE_ACCOUNT_EMAIL', 'GOOGLE_CLIENT_EMAIL']),
    FIREBASE_KEY_LEGACY: hasValue(env, 'FIREBASE_KEY')
  };
  return {
...
function validateRuntimeEnv(env = process.env) {
  ...
  if (production && hasValue(env, 'PUBLIC_API_BASE')) {
    const originError = validateOriginForProduction(normalizeUrlOrigin(env.PUBLIC_API_BASE));
