...
function validateRuntimeEnv(env = process.env) {
  ...
  if (production && !hasPreferredFirebaseCredential && !hasSplitFirebaseCredential && !hasLegacyRawFirebaseKey) {
    errors.push('Üretimde Firebase Admin credential zorunludur: FIREBASE_KEY_BASE64, FIREBASE_KEY_PATH veya split FIREBASE_PRIVATE_KEY + FIREBASE_CLIENT_EMAIL kullan. Geçici uyumluluk için FIREBASE_KEY okunabilir.');
  }
