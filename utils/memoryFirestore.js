...
class MemoryQuery {
  constructor(store, collectionPath, filters = [], order = [], limitValue = 0, startAfterValue = undefined, options = {}) {
    this._store = store;
  ...
  _next(filters = this._filters, order = this._order, limitValue = this._limitValue, startAfterValue = this._startAfterValue) {
    return new MemoryQuery(this._store, this._collectionPath, filters, order, limitValue, startAfterValue, this._options);
  }

  where(field, op, value) {
    return this._next([...this._filters, { field: normalizeFieldKey(field), op: String(op || '=='), value }]);
  }

  orderBy(field, direction = 'asc') {
    return this._next(this._filters, [...this._order, { field: normalizeFieldKey(field), direction: String(direction || 'asc').toLowerCase() }]);
  }

  limit(value) {
    return this._next(this._filters, this._order, Math.max(0, Number(value) || 0));
  }

  startAfter(value) {
    return this._next(this._filters, this._order, this._limitValue, value);
  }
  ...
  _collectionDocs() {
    const docs = [];
    const groupId = this._options.collectionGroup ? this._collectionPath : '';
    const prefix = !groupId && this._collectionPath ? `${this._collectionPath}/` : '';
    const baseDepth = !groupId && this._collectionPath ? this._collectionPath.split('/').length : 0;
    for (const [path, data] of this._store._documents.entries()) {
      const parts = path.split('/').filter(Boolean);
      if (parts.length % 2 !== 0) continue;
      if (groupId) {
        if (parts.length < 2 || parts[parts.length - 2] !== groupId) continue;
      } else {
...
class MemoryFirestore {
  ...
  collectionGroup(id) {
    const safeId = cleanPath(id);
    if (!safeId || safeId.includes('/')) throw new Error('collectionGroup id tek segment olmalıdır');
    return new MemoryQuery(this, safeId, [], [], 0, undefined, { collectionGroup: true });
  }
