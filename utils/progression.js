...
const ACCOUNT_PROGRESSION_VERSION = 2;
const LEGACY_ACCOUNT_XP_STEP = 100;
const LEGACY_RP_TO_XP_MULTIPLIER = 10;

const ACCOUNT_LEVEL_THRESHOLDS = Object.freeze((() => {
  const table = new Array(ACCOUNT_LEVEL_CAP + 1).fill(0);
  let cumulativeXp = 0;
  let stepXp = ACCOUNT_XP_STEP;
  table[1] = 0;
  for (let level = 2; level <= ACCOUNT_LEVEL_CAP; level += 1) {
    if (level > 2) stepXp *= (level - 1);
    cumulativeXp += stepXp;
...
function deriveLegacyQuadraticXpFromLevel(level = 1) {
  const safeLevel = normalizeLevel(level);
  return Math.max(0, Math.round(Math.pow(Math.max(0, safeLevel - 1), 2) * LEGACY_ACCOUNT_XP_STEP));
}

function getLegacyLevelFromStoredXp(xp = 0) {
  const safeXp = Math.max(0, safeNum(xp, 0));
  const computed = Math.floor(Math.sqrt(safeXp / LEGACY_ACCOUNT_XP_STEP)) + 1;
  return normalizeLevel(computed);
}

function mapLegacyXpToCurrentXp(legacyXp = 0) {
  const safeLegacyXp = Math.max(0, safeNum(legacyXp, 0));
  const legacyLevel = getLegacyLevelFromStoredXp(safeLegacyXp);
  const legacyLevelXp = deriveLegacyQuadraticXpFromLevel(legacyLevel);
  const legacyNextLevelXp = legacyLevel >= ACCOUNT_LEVEL_CAP ? legacyLevelXp : deriveLegacyQuadraticXpFromLevel(legacyLevel + 1);
  const legacySpan = Math.max(1, legacyNextLevelXp - legacyLevelXp);
  const progressPct = legacyLevel >= ACCOUNT_LEVEL_CAP ? 1 : Math.max(0, Math.min(1, (safeLegacyXp - legacyLevelXp) / legacySpan));
  const currentLevelXp = deriveXpFromLevel(legacyLevel);
  const currentNextLevelXp = legacyLevel >= ACCOUNT_LEVEL_CAP ? currentLevelXp : deriveXpFromLevel(legacyLevel + 1);
  const currentSpan = Math.max(1, currentNextLevelXp - currentLevelXp);
  return Math.max(0, currentLevelXp + Math.round(currentSpan * progressPct));
}

function deriveLegacyXpFromRp(rp = 0) {
  const legacyXp = Math.max(0, Math.round(safeNum(rp, 0) * LEGACY_RP_TO_XP_MULTIPLIER));
  return mapLegacyXpToCurrentXp(legacyXp);
}
...
function isCurrentProgressionVersion(user = {}) {
  return Math.floor(safeNum(user.accountProgressionVersion, 0)) >= ACCOUNT_PROGRESSION_VERSION;
}
...
function getAccountXp(user = {}) {
  ...
  if (Number.isFinite(explicitAccountXp) && explicitAccountXp >= 0) return mapLegacyXpToCurrentXp(explicitAccountXp);
  if (Number.isFinite(explicitXp) && explicitXp >= 0) return mapLegacyXpToCurrentXp(explicitXp);
  if (Number.isFinite(explicitLevel) && explicitLevel > 0) return deriveXpFromLevel(explicitLevel);
  return deriveLegacyXpFromRp(getCompetitiveScore(user));
}
...
function getAccountLevel(user = {}) {
  const explicitLevel = safeNum(user.level, Number.NaN);
  if (isCurrentProgressionVersion(user) && Number.isFinite(explicitLevel) && explicitLevel > 0) {
    return normalizeLevel(explicitLevel);
  }
