...
async function respondToInvite(inviteId = '', responderUid = '', response = '') {
  ...
  return db.runTransaction(async (tx) => {
    ...
    const responseMessage = nextStatus === 'accepted' ? 'Davet kabul edildi.' : 'Davet reddedildi.';
    tx.set(ref, { status: nextStatus, respondedAt: nowMs(), updatedAt: nowMs(), closeReason: nextStatus === 'accepted' ? 'accepted' : 'declined', statusMessage: responseMessage, deliveryStatus: 'closed' }, { merge: true });
    return { inviteId: safeInviteId, ...data, status: nextStatus, respondedAt: nowMs(), closeReason: nextStatus === 'accepted' ? 'accepted' : 'declined', statusMessage: responseMessage, deliveryStatus: 'closed' };
  });
}
  ...
  while (true) {
    ...
    for (const doc of inviteSnap.docs || []) {
      ...
      if (status === 'pending' && expiresAt > 0 && expiresAt <= now) {
        inviteBatch.set(doc.ref, { status: 'expired', respondedAt: now, updatedAt: now, closeReason: 'ttl_expired', statusMessage: 'Davet süresi dolduğu için kapandı.', deliveryStatus: 'closed' }, { merge: true });
        inviteOps += 1;
