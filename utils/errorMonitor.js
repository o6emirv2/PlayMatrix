...
const { storeServerErrorObservation } = require('./liveObservationStore');
...
async function captureError(error, context = {}) {
  ...
  try {
    await storeServerErrorObservation(err, {
      scope: cleanStr(context.scope || 'server', 80),
      path: cleanStr(context.path || '', 180),
      requestId: cleanStr(context.requestId || '', 120),
      uid: cleanStr(context.uid || '', 180),
      userAgent: cleanStr(context.userAgent || '', 240),
